/* Stretched image spans: prove the stepped source-index table and the arm that
 * divides per pixel produce the same panel, in one binary, on a scene rebuilt
 * from scratch for each arm (the shape test_image_rotate_arms.c uses). Every
 * configuration runs a REPLACE frame and then PATCH frames that move the
 * destination rectangle AND resize it, so the stretch ratio, the span anchor
 * and every span boundary change underneath the stepping. The provider counts
 * its fetches, so the two arms must also ask for the same blocks of source
 * pixels, not merely land on the same colours, and one magnification scene
 * asserts the span count the stretch path is supposed to produce (a switch that
 * is never taken would otherwise compare the division path with itself). */
#include "core_fixture.h"
#include "ksn_render.h"
#include <assert.h>
#include <stdio.h>
#include <string.h>

#define PANEL (240*135)
static uint16_t panel[PANEL],strip[240*8];
static unsigned long long fetches;
static uint16_t *buffer(void *p){(void)p;return strip;}
static ksn_result send(void *p,uint16_t y,uint16_t rows,const uint16_t *pixels){
    (void)p;memcpy(panel+y*240,pixels,rows*240*2);return KSN_OK;}
static uint16_t source_color(unsigned x,unsigned y,unsigned frame){return (uint16_t)(x*313+y*937+frame*3001);}
static uint8_t source_coverage(unsigned x,unsigned y){const uint8_t a[]={0,1,127,128,254,255};return a[(x+y*3)%6];}
static ksn_result source(void *p,uint16_t variant,uint16_t frame,uint16_t y,uint16_t x,uint16_t n,uint16_t *rgb,uint8_t *alpha){
    (void)p;assert(variant<1&&frame<2&&y<160&&x+n<=160&&n<=32);
    fetches++;
    for(unsigned i=0;i<n;i++){rgb[i]=source_color(x+i,y,frame);alpha[i]=source_coverage(x+i,y);}
    return KSN_OK;
}
static uint32_t hash_bytes(const uint16_t *pixels,unsigned count){
    uint32_t h=2166136261u;const uint8_t *bytes=(const uint8_t *)pixels;
    for(unsigned i=0;i<count*2;i++){h^=bytes[i];h*=16777619u;}
    return h;
}
static unsigned long long panel_diff(const uint16_t *a,const uint16_t *b,unsigned long long *worst){
    unsigned long long n=0;
    for(unsigned i=0;i<PANEL;i++)if(a[i]!=b[i]){
        unsigned long long d=a[i]>b[i]?(unsigned long long)(a[i]-b[i]):(unsigned long long)(b[i]-a[i]);
        if(d>*worst)*worst=d;
        n++;
    }
    return n;
}

typedef struct {
    int x,y,w,h;              /* destination bounds, the stretch rectangle */
    unsigned source_x,source_y,source_w,source_h;
    unsigned opacity;         /* 0..255 */
    int group;                /* isolated group opacity, or the direct path */
    int toggle_frame;         /* every other PATCH changes the image frame */
    ksn_rect clip;
    unsigned steps;           /* PATCH frames after the REPLACE frame */
} config;

/* One scene, one arm. Returns a hash over every rendered frame and leaves the
 * last frame in `panel`. */
static uint32_t scene(const config *c,bool step,unsigned long long *fetch_count){
    KSN_TEST_CORE(core,);ksn_core_init(&core);
    ksn_client app=ksn_core_client(&core,KSN_APP);
    ksn_image_port port={NULL,160,160,1,2,source};ksn_resource image;
    assert(ksn_core_register_image(&core,KSN_APP,&port,&image)==KSN_OK);
    ksn_display_port display={NULL,buffer,send,240,135,8,NULL};
    ksn_render_stats stats;ksn_tx tx;ksn_ref ref;uint32_t hash=2166136261u;
    ksn_draw d={.kind=KSN_IMAGE,.clip=c->clip,.opacity=(uint8_t)c->opacity,
        .data.image={.resource=image,.scale=KSN_IMAGE_STRETCH,.source_x=(uint16_t)c->source_x,
                     .source_y=(uint16_t)c->source_y,.source_width=(uint16_t)c->source_w,
                     .source_height=(uint16_t)c->source_h,.rotation=0}};
    d.bounds=(ksn_rect){(int16_t)c->x,(int16_t)c->y,(int16_t)(c->x+(int)c->w),(int16_t)(c->y+(int)c->h)};
    g_ksn_image_stretch_step=step;g_ksn_image_rotate_step=true;
    fetches=0;
    assert(app.ops->begin(app.ctx,KSN_REPLACE,&tx)==KSN_OK);
    assert(app.ops->background(app.ctx,tx,0x183c60ff)==KSN_OK);
    assert(app.ops->add(app.ctx,tx,&d,&ref)==KSN_OK);
    if(c->group)assert(ksn_core_group(&core,KSN_APP,tx,ref,1,(uint8_t)(137+(c->group&1)))==KSN_OK);
    assert(app.ops->end(app.ctx,tx)==KSN_OK);
    assert(ksn_render_rects(&core,&display,&stats)==KSN_OK);
    hash^=hash_bytes(panel,PANEL);hash*=16777619u;
    for(unsigned s=1;s<=c->steps;s++){
        /* Move and resize: the destination width changes, so the mapping's
         * denominator does, and every span boundary with it. */
        d.bounds=(ksn_rect){(int16_t)(c->x+(int)(s*11)-70),(int16_t)(c->y+(int)(s*3)-12),
                            (int16_t)(c->x+(int)(s*11)-70+(int)c->w+(int)s),
                            (int16_t)(c->y+(int)(s*3)-12+(int)c->h+(int)(s*7))};
        assert(app.ops->begin(app.ctx,KSN_PATCH,&tx)==KSN_OK);
        ksn_change change={.property=KSN_SET_RECT,.value.rect=d.bounds};
        assert(app.ops->change(app.ctx,tx,ref,&change)==KSN_OK);
        if(c->toggle_frame){
            d.data.image.frame=(uint8_t)(s&1u);
            change=(ksn_change){.property=KSN_SET_IMAGE_FRAME,.value.image={0,(uint8_t)(s&1u)}};
            assert(app.ops->change(app.ctx,tx,ref,&change)==KSN_OK);
        }
        assert(app.ops->end(app.ctx,tx)==KSN_OK);
        assert(ksn_render_rects(&core,&display,&stats)==KSN_OK);
        hash^=hash_bytes(panel,PANEL);hash*=16777619u;
    }
    *fetch_count=fetches;
    g_ksn_image_stretch_step=true;
    return hash;
}
/* The 120-frame animated stretch track: position and scale, no rotation, 30 fps,
 * looping. `arm_mode` 0 steps, 1 divides, 2 alternates per frame, so a
 * dependence on which arm rendered the previous frame cannot hide. */
static uint32_t animate_120(unsigned arm_mode,unsigned long long *fetch_count,unsigned *frames_out){
    KSN_TEST_CORE(core,);ksn_core_init(&core);
    static ksn_core_animation_block blocks[2];
    assert(ksn_core_enable_animation(&core,&blocks[0],&blocks[1])==KSN_OK);
    assert(ksn_core_animation_bytes(&core)<=1024);
    ksn_client app=ksn_core_client(&core,KSN_APP);
    ksn_image_port port={NULL,160,160,1,2,source};ksn_resource image;
    assert(ksn_core_register_image(&core,KSN_APP,&port,&image)==KSN_OK);
    ksn_display_port display={NULL,buffer,send,240,135,8,NULL};
    ksn_render_stats stats;ksn_tx tx;ksn_ref ref;uint32_t hash=2166136261u;unsigned frames=0;
    ksn_draw d={.kind=KSN_IMAGE,.clip={0,0,240,135},.opacity=193,.bounds={10,20,74,84},
        .data.image={.resource=image,.scale=KSN_IMAGE_STRETCH,.source_x=3,.source_y=2,
                     .source_width=64,.source_height=64,.rotation=0}};
    ksn_motion motion={.count=1,.property=KSN_TRANSFORM,.from.pose={{10,20,74,84},0},
        .to.pose={{150,40,232,122},0},.duration_ms=1000,.easing=KSN_LINEAR,.repeat=KSN_LOOP};
    g_ksn_image_stretch_step=true;g_ksn_image_rotate_step=true;
    fetches=0;
    assert(app.ops->begin(app.ctx,KSN_REPLACE,&tx)==KSN_OK);
    assert(app.ops->background(app.ctx,tx,0x183c60ff)==KSN_OK);
    assert(app.ops->add(app.ctx,tx,&d,&ref)==KSN_OK);
    motion.first=ref;
    ksn_animation animation;
    assert(app.ops->animate(app.ctx,tx,&motion,&animation)==KSN_OK);
    assert(app.ops->end(app.ctx,tx)==KSN_OK);
    assert(ksn_render_rects(&core,&display,&stats)==KSN_OK);
    hash^=hash_bytes(panel,PANEL);hash*=16777619u;
    ksn_core_start_animations(&core,0);
    for(unsigned f=1;f<=120;f++){
        ksn_tx atx={0};
        assert(ksn_core_advance_animations(&core,(uint64_t)f*33334u,false,&atx)==KSN_OK);
        if(!atx.value)continue;
        frames++;
        g_ksn_image_stretch_step=arm_mode==2?(f&1u)!=0:(arm_mode==0);
        assert(ksn_render_rects(&core,&display,&stats)==KSN_OK);
        hash^=hash_bytes(panel,PANEL);hash*=16777619u;
    }
    *fetch_count=fetches;*frames_out=frames;
    g_ksn_image_stretch_step=true;
    return hash;
}
static void both_arms(const config *c,const char *what,unsigned index,
                      unsigned long long *compared,unsigned long long *worst){
    static uint16_t first[PANEL];
    unsigned long long fa,fb;
    uint32_t ha=scene(c,true,&fa);memcpy(first,panel,sizeof(panel));
    uint32_t hb=scene(c,false,&fb);
    (*compared)+=2;
    if(ha!=hb||memcmp(first,panel,sizeof(panel))!=0||fa!=fb){
        unsigned long long n=panel_diff(first,panel,worst);
        printf("image stretch arms MISMATCH %s=%u diff=%llu fetches=%llu/%llu hash=%08x/%08x\n",
            what,index,n,fa,fb,ha,hb);assert(false);}
}
int main(void){
    unsigned long long compared=0,configs=0,worst_step=0;
    config c={.x=50,.y=30,.w=64,.h=64,.source_x=3,.source_y=2,.source_w=64,.source_h=64,
              .opacity=255,.group=0,.toggle_frame=0,.clip={0,0,240,135},.steps=2};
    /* 1. The destination size and the source window, so the ratio runs from
     *    heavy minification to full-panel magnification. */
    {const unsigned sizes[][2]={{1,1},{2,3},{17,9},{32,32},{33,64},{64,64},{100,50},{240,135}};
     const unsigned windows[][2]={{1,1},{3,2},{16,16},{64,32},{96,96},{160,160}};
     for(unsigned s=0;s<sizeof(sizes)/sizeof(sizes[0]);s++)
         for(unsigned w=0;w<sizeof(windows)/sizeof(windows[0]);w++){
             c.x=8;c.y=6;c.w=(int)sizes[s][0];c.h=(int)sizes[s][1];
             c.source_x=0;c.source_y=0;c.source_w=windows[w][0];c.source_h=windows[w][1];
             both_arms(&c,"size",s*100+w,&compared,&worst_step);
             configs++;
         }}
    /* 2. Destination and source origins, so the 16-column block grid and the
     *    span phase start off the boundary. */
    for(unsigned ox=0;ox<4;ox++)for(unsigned oy=0;oy<4;oy++){
        c.x=37;c.y=17;c.w=64;c.h=64;
        c.source_x=ox?ox*15u+1u:0u;c.source_y=oy?oy*15u+1u:0u;
        c.source_w=96;c.source_h=96;
        both_arms(&c,"origin",ox*4+oy,&compared,&worst_step);
        configs++;
    }
    c.source_x=3;c.source_y=2;c.source_w=64;c.source_h=64;
    /* 3. Clip, opacity and isolated group opacity, both render paths. */
    {const ksn_rect clips[]={{0,0,240,135},{9,11,225,122},{50,30,60,40},{120,60,240,135}};
     const unsigned opacities[]={0,1,127,128,193,254,255};
     for(unsigned ci=0;ci<4;ci++)for(unsigned oi=0;oi<7;oi++)for(unsigned g=0;g<2;g++){
         c.x=45;c.y=25;c.w=64;c.h=64;c.clip=clips[ci];c.opacity=opacities[oi];c.group=(int)g;
         both_arms(&c,"clip",ci*100+oi*10+g,&compared,&worst_step);
         configs++;
     }}
    c.clip=(ksn_rect){0,0,240,135};c.opacity=193;
    /* 4. A stretched image entering the panel from off-screen, 20 PATCH frames
     *    per configuration. */
    c.steps=20;
    for(unsigned origin=0;origin<3;origin++)for(unsigned group=0;group<2;group++){
        c.x=-64+(int)origin*30;c.y=-32+(int)origin*10;c.w=64;c.h=64;c.group=(int)group;
        both_arms(&c,"motion",origin*2+group,&compared,&worst_step);
        configs++;
    }
    /* 5. Frame changes under a stretch: the same rectangle fetches another row. */
    c.steps=8;
    for(unsigned group=0;group<2;group++){
        c.x=20;c.y=12;c.w=80;c.h=40;c.group=(int)group;c.toggle_frame=1;
        both_arms(&c,"frame",group,&compared,&worst_step);
        configs++;
    }
    c.toggle_frame=0;c.steps=2;
    /* 6. Non-vacuity of the path under test: a 160x160 window stretched over the
     *    whole panel is magnification, so each 16-column destination span needs
     *    16/1.5 source columns, never the 32-pixel clamp, and the number of
     *    provider fetches is exactly one per span. */
    {unsigned long long fa=0;
     c.x=0;c.y=0;c.w=240;c.h=135;c.source_x=0;c.source_y=0;c.source_w=160;c.source_h=160;
     c.steps=0;c.clip=(ksn_rect){0,0,240,135};c.opacity=255;c.group=0;
     scene(&c,true,&fa);
     assert(fa==(unsigned long long)15*135);
     printf("stretched panel: %llu spans for 240x135 destination pixels\n",fa);
     configs++;}
    /* 7. 120-frame animated stretch track, both arms and an alternating arm. */
    {unsigned long long f_step,f_div,f_alt;unsigned n_step,n_div,n_alt;
     static uint16_t first[PANEL];
     uint32_t h_step=animate_120(0,&f_step,&n_step);memcpy(first,panel,sizeof(panel));
     uint32_t h_div=animate_120(1,&f_div,&n_div);
     unsigned long long diff=panel_diff(first,panel,&worst_step);
     compared+=2;
     if(h_step!=h_div||diff||f_step!=f_div||n_step!=120||n_div!=120){
         printf("image stretch arms MISMATCH animated frames=%u/%u diff=%llu fetches=%llu/%llu hash=%08x/%08x\n",
             n_step,n_div,diff,f_step,f_div,h_step,h_div);assert(false);}
     uint32_t h_alt=animate_120(2,&f_alt,&n_alt);
     compared+=1;
     if(h_alt!=h_step||f_alt!=f_step){
         printf("image stretch arms MISMATCH alternating frames=%u fetches=%llu hash=%08x/%08x\n",
             n_alt,f_alt,h_alt,h_step);assert(false);}
     assert(f_step>0);
     printf("animated stretch track: %u frames both arms, alternating arm identical, %llu fetches\n",n_step,f_step);
     configs+=2;}
    printf("image stretch arms PASS: %llu configs, %llu panel hashes, worst pixel step %llu, fetches identical\n",
        configs,compared,worst_step);
    return 0;
}
