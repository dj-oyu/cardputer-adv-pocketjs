...
static uint32_t scene(const config *c,bool step,unsigned long long *fetch_count){
    ...
    *fetch_count=fetches;
    g_ksn_image_rotate_step=true;
    return hash;
}
/* The 120-frame animated transform track: position, scale and multi-turn
 * rotation, 30 fps, looping. `arm_mode` 0 steps, 1 divides, 2 alternates per
 * frame, so a dependence on which arm rendered the previous frame cannot hide. */
static uint32_t animate_120(unsigned arm_mode,unsigned long long *fetch_count,unsigned *frames_out){
    ...
    for(unsigned f=1;f<=120;f++){
        ...
        frames++;
        g_ksn_image_rotate_step=arm_mode==2?(f&1u)!=0:(arm_mode==0);
        assert(ksn_render_rects(&core,&display,&stats)==KSN_OK);
        hash^=hash_bytes(panel,PANEL);hash*=16777619u;
    }
...
int main(void){
    ...
    /* 1. Every rotation at a fixed pose. */
    for(unsigned rotation=0;rotation<1024;rotation++){
        ...
        if(ha!=hb||memcmp(first,panel,sizeof(panel))!=0||fa!=fb){
            unsigned long long n=panel_diff(first,panel,&worst_step);
            printf("image rotate arms MISMATCH rotation field=%u diff=%llu fetches=%llu/%llu hash=%08x/%08x\n",
                rotation,n,fa,fb,ha,hb);assert(false);}
        configs++;
    }
     ...
     for(unsigned s=0;s<sizeof(sizes)/sizeof(sizes[0]);s++)
         for(unsigned w=0;w<sizeof(windows)/sizeof(windows[0]);w++)
             for(unsigned rotation=0;rotation<1024;rotation+=37){
                 ...
                 if(ha!=hb||memcmp(first,panel,sizeof(panel))!=0||fa!=fb){
                     unsigned long long n=panel_diff(first,panel,&worst_step);
                     printf("image rotate arms MISMATCH size field=%u rotation=%u diff=%llu fetches=%llu/%llu\n",
                         s*100+w,rotation,n,fa,fb);assert(false);}
                 configs++;
             }}
    ...
    for(unsigned ox=0;ox<4;ox++)for(unsigned oy=0;oy<4;oy++)
        for(unsigned rotation=0;rotation<1024;rotation+=97){
            ...
            if(ha!=hb||memcmp(first,panel,sizeof(panel))!=0||fa!=fb){
                unsigned long long n=panel_diff(first,panel,&worst_step);
                printf("image rotate arms MISMATCH origin field=%u rotation=%u diff=%llu fetches=%llu/%llu\n",
                    ox*4+oy,rotation,n,fa,fb);assert(false);}
            configs++;
        }
    ...
    {const ksn_rect clips[]={{0,0,240,135},{9,11,225,122},{50,30,60,40},{120,60,240,135}};
     ...
     for(unsigned ci=0;ci<4;ci++)for(unsigned oi=0;oi<7;oi++)for(unsigned g=0;g<2;g++)
         for(unsigned r=0;r<7;r++){
             ...
             if(ha!=hb||memcmp(first,panel,sizeof(panel))!=0||fa!=fb){
                 unsigned long long n=panel_diff(first,panel,&worst_step);
                 printf("image rotate arms MISMATCH clip field=%u rotation=%u diff=%llu fetches=%llu/%llu\n",
                     ci*100+oi,rotations[r],n,fa,fb);assert(false);}
             configs++;
         }}
    /* 5. Long motion: an off-screen image entering the panel, rotation turning
     *    with it, 20 PATCH frames per configuration. */
    ...
    for(unsigned origin=0;origin<3;origin++)for(unsigned group=0;group<2;group++){
        ...
        if(ha!=hb||memcmp(first,panel,sizeof(panel))!=0||fa!=fb){
            unsigned long long n=panel_diff(first,panel,&worst_step);
            printf("image rotate arms MISMATCH motion origin=%u group=%u diff=%llu fetches=%llu/%llu\n",
                origin,group,n,fa,fb);assert(false);}
        configs++;
    }
    /* 6. 120-frame animated track, both arms and an alternating arm. */
    {unsigned long long f_step,f_div,f_alt;unsigned n_step,n_div,n_alt;
     ...
     if(h_alt!=h_step||f_alt!=f_step){
         printf("image rotate arms MISMATCH alternating frames=%u fetches=%llu hash=%08x/%08x\n",
             n_alt,f_alt,h_alt,h_step);assert(false);}
     printf("animated track: %u frames both arms, alternating arm identical, %llu fetches\n",n_step,f_step);
     configs+=2;}
