...
#
#   wsl -e bash tools/vmtest/floor/floor32.sh
set -e
cd "$(dirname "$0")/../../.."
...
mkdir -p "$OUT"
VMTEST_OUT=$OUT VMTEST_CFLAGS="$F" bash tools/vmtest/build.sh o2 > "$OUT/floor32_build.log" 2>&1 || { tail -20 "$OUT/floor32_build.log"; exit 1; }
