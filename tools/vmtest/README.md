...
| `l2b_async_flat.js` | L2b-async（設計 §12.2、D32〜D34）。JS から呼んだ async 関数の最初の同期区間がフラットフレームで走り、promise が呼び出し元の func スロットを経て戻る経路を 1 行ずつ固定する: promise の同一性とクラス、同期区間と呼び出し元の次の文の順序、最初の await 前の throw が拒否済み promise になり呼び出し元の `try` に届かないこと、method 呼び出しの `this`、本体で読む rest / `arguments` / 既定引数、async アロー・クラスの async メソッド・`return await`、本体からの `Error().stack` の関数名、tail 位置（`return f()`）、async から async、generator 床からの既定引数初期化子経由の呼び出し（§10.3）、await 復帰後の再帰、C 再帰のまま囲った経路（apply / call / bound / Proxy / Reflect / 組み込みのコールバック）、誰も持たない promise。**`asan-recur`（上流経路）で bless し、flat がバイト一致する**。深い再帰は含めない（`budget_probe.sh` の `deep_async_recursion` の側） |
...
有限frameの対照はSELFTEST専用USB `[` / `\` / `]`（2万/4万/10万回の整数加算）。
`device_finite_frame.py --port COM3`が完走時の総和またはframe暴走ガードによる停止を採取する。
SELFTEST/YIELDの新しいimageでは`--timing`を付け、総和だけでなくframeが正常に戻った後の
`VM_FRAME_COMPLETE`まで待つ。完走にも`execution_us`が付く。これはJS呼出しと再開区間の
累積壁時計時間（プリエンプションとnative完了ログを含む）で、park中の待機は含まず、CPU時間ではない。
計測ログ自体は時間取得後に出力する。通常ビルドには計測用フラグ・分岐・ログを追加しない。
互換/FAIR構成の各3回の実測では2万/4万が完走し10万が停止（results §4.17）。
別構成までこの結果を固定せず、全負荷の完走を要求する検査にはしていない。
これは観測ツールで、停止した有限処理を「正常負荷が成功した」と数えてはいけない。
ホスト側の`test_device_finite_frame.py`は総和・起点違い・欠落・矛盾を検査する。
