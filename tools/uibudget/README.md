...
**taffyノード = ルート + 木に繋がっている全View + 本文が空でないText。** 空になるのは `NodeType::Text` のときだけで（`layout.rs:256`）、Viewは無条件に `new_with_children` へ入ります。**テキストだけ数えると足りません** — ペットアプリはViewを8個持つため、Textだけなら10個で安全に見えて、実際は18でした。

`main/pocket_ui.c` の `layout_block()` は生存ノードを全部数えるので**多めに見積もり、崖の手前で断ります**。ガードが間違えるならこの向きでなければいけません。
