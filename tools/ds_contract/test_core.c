...
int main(void){
    ...
    CHECK(limits.native_bytes==DS_CORE_STORAGE_BYTES);
    printf("fixed core storage: %lu bytes\n",(unsigned long)limits.native_bytes);
    ...
    CHECK(ds_core_presented(&core)==DS_OK&&!ds_core_has_submission(&core));
    CHECK(ds_core_active_usage(&core,DS_APP).commands==3);

    CHECK(pet_view_update(app,&view,100)==DS_OK);
    CHECK(ds_core_presented(&core)==DS_OK);
    ...
    CHECK(app.ops->end(app.ctx,tx)==DS_OK&&ds_core_presented(&core)==DS_OK);
