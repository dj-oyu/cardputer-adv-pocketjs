...
void reference_init(ds_frost *frost){if(frost)memset(frost,0,sizeof(*frost));}
ds_result reference_feed(ds_frost *frost,uint16_t y,uint16_t rows,const uint16_t *pixels){
...
ds_result reference_blur(ds_frost *frost,uint8_t radius){
...
ds_result reference_span(const ds_frost *frost,uint16_t y,uint16_t x,uint16_t count,
