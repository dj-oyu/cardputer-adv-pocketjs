...
/* VMALLOC PATCH (tools/vmalloc, not applied to the firmware build):
 * IDF hardcodes ALIGN_SIZE_LOG2=2 (4 B), matching a 32-bit Xtensa target
 * where block_header_t's prev_phys_block pointer is 4 B. On this x86-64
 ...
 * the block-search bucket boundaries the algorithm evaluates against, not
 * the fit strategy or split/coalesce logic, which are unmodified below. */
#ifndef VMALLOC_TLSF_ALIGN_LOG2
#define VMALLOC_TLSF_ALIGN_LOG2 2
#endif

enum tlsf_config
{
	/* All allocation sizes and addresses are aligned to 4 bytes. */
	ALIGN_SIZE_LOG2 = VMALLOC_TLSF_ALIGN_LOG2,
...
typedef struct control_t
{
    ...
    /* VMALLOC PATCH: widened from :3 to :4 and small_block_size from :8 to
     * :9 below. With ALIGN_SIZE_LOG2 fixed at 2, fl_index_shift maxes out
     * at sl_index_count_log2(5)+2=7 (fits :3) and small_block_size at
     * 1<<7=128 (fits :8) - IDF sized these bitfields exactly to that case
     ...
     * behavior at the original ALIGN_SIZE_LOG2=2 - only removes truncation
     * that the original packing never had to handle. */
    unsigned int fl_index_shift : 4; // 9 cumulated bits
    unsigned int fl_index_max : 6; // 15 cumulated bits
    unsigned int sl_index_count : 6; // 21 cumulated bits

	/* log2 of number of linear subdivisions of block sizes. Larger
	** values require more memory in the control structure. Values of
	** 4 or 5 are typical.
	*/
    unsigned int sl_index_count_log2 : 3; // 24 cumulated bits
    unsigned int small_block_size : 9; // 33 cumulated bits
