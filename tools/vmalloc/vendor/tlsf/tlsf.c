...
static control_t* control_construct(control_t* control, size_t bytes)
{
	...
	/* VMALLOC PATCH: pad to ALIGN_SIZE. sl_bitmap is an array of `unsigned
	 * int` (4 B) times fl_index_count, which is often odd (e.g. 21), so the
	 * running total can land 4 mod 8. At the stock ALIGN_SIZE=4 this is
	 * already a multiple of 4 and the pad is a no-op; tlsf_size(tlsf) is
	 * this value added straight to a pool pointer by tlsf_create_with_pool,
	 * so at VMALLOC_TLSF_ALIGN_LOG2=3 the unpadded total produced a 4 mod 8
	 * pool start and tripped tlsf_add_pool's own alignment check. */
	control->size = (control->size + (ALIGN_SIZE - 1)) & ~(size_t)(ALIGN_SIZE - 1);
