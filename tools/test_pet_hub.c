...
int main(int argc,char **argv) {
    pet_hub_t h;reset_hub(&h);uint8_t d[48];char label[25];
    ...
    pet_hub_saved_t saved=h.saved;reset_hub(&h);h.saved=saved;
    ...
    reset_hub(&h);h.saved.utc_offset=0;h.saved.wake_minute=420;
    ...
    if(argc>1){FILE *f=fopen(argv[1],"rb");assert(f);assert(fread(d,1,48,f)==48);fclose(f);reset_hub(&h);assert(pet_hub_packet(&h,d));assert(h.saved.usage[1].tokens==12345);}
