...
static unsigned long visits,miss_d,miss_depth,shaded,bell_visits,bell_miss;
...
int main(void){
    ...
    for(int sp=1;sp<=7;sp++){
        visits=miss_d=miss_depth=shaded=bell_visits=bell_miss=0;
        for(int f=0;f<30;f++){
        ...
        printf("%-10s visits/frame %7lu  ellipsoid %6lu (d<0 %4.1f%%  behind %4.1f%%  shaded %4.1f%%)  bell %6lu (miss %4.1f%%)\n",
