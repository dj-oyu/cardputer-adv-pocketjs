// USB diagnostic x. The audio source pins two early pool generations only in
// the copy-probe build, forcing bounded BUSY/skip while MP3 output continues.
(function () {
  ...
  const source = pocket.audio.outputSource(true);
  let bound = false;
