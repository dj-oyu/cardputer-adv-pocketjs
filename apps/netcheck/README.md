...
## tlscycle.js — 断片化からの復帰

`tlsleak.js` が見つけた8回の壁は、**リースを回せば消える**。

```
8回目のあと   free 39,660  block 29,696
close → 3秒 → acquire
9回目の直前   free 57,104  block 31,744   ← 初回と同じ
その後8回すべて 200
```

空きも最大連続ブロックも完全に戻る。断片化は net スタックが無線停止時に返す
確保の中だけで起きていて、リースをまたいで蓄積するものは無い。

**アプリ側の指針: 長く動くアプリは8回程度ごとにリースを閉じて取り直す。**
自動ではやらない — アプリのリンクを黙って3秒落とすのは、アプリが選ぶべき
ことであって発見させることではない。

恒久的な解は接続の保持（keep-alive）で、TLSコンテキストを毎回作り直さない形。
これは §11 に記述が無いので仕様の追加が要る。
