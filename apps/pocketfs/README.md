...
- 索引は**最初のファイル操作のときだけ** 内部DRAMから **3080 bytes** を取り、
  `app_stop()` で返す。ファイルを触らないアプリのコストは0で、触ったアプリも
  終了時に返す。値は `pocket_fs.c` の `_Static_assert` でビルド時に固定。
  writerを開いている間だけ 4072 bytes のステージングバッファが1本増える。
  セッションをまたいで常駐するのは `.bss`+`.data` の **449 bytes** だけ。
