 ...
 * predicate path runs. `x` is the tile-local column and `color` is what
 * `sample` returns for that pixel, from the row table when one was built. */
static void group_pixel(ksn_premultiplied_rgba8 *tile,const uint8_t *coverage,int x,
                        const ksn_frame_view *command,ksn_rgba color,bool has_dither,
            ...
            if(g_ksn_row_coverage){
                ...
                for(unsigned run=0;run<run_count;run++){
                    ...
                    if(row)for(int px=from;px<to;px++)
                        group_pixel(tile,coverage,px-x0,command,row_table_value(row,px),
                                    has_dither,child_dither,dither_pixels);
                    else for(int px=from;px<to;px++)
                        group_pixel(tile,coverage,px-x0,command,sample(command,px,py),
                                    has_dither,child_dither,dither_pixels);
                }
            }else for(int x=0;x<count;x++)if(covers(command,x0+x,py))
                group_pixel(tile,coverage,x,command,sample(command,x0+x,py),has_dither,
                            child_dither,dither_pixels);
            KSN_PROF_END(blend);}
        }
        {KSN_PROF_BEGIN();
