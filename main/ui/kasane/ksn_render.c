...
static ksn_result render_group(ksn_core *core,const ksn_text_port *text,ksn_tx ticket,ksn_layer layer,unsigned first,unsigned end,
    ...
    if(g_ksn_group_affine>=2){
        ...
        for(int py=top;py<bottom;py++)for(int x0=left;x0<right;x0+=32){
            ...
            ksn_result result=group_folded_chunk(core,text,ticket,layer,first,end,opacity,py,x0,y,
                                                 count,coverage,has_dither,&scratch.acc,pixels);
            if(result!=KSN_OK)return result;
        }
        KSN_PROF_END(blend);}
        return KSN_OK;
    ...
    for(int py=top;py<bottom;py++)for(int x0=left;x0<right;x0+=64){
        ...
        memset(scratch.tile,0,sizeof(scratch.tile));
            ...
            if(g_ksn_row_coverage){
                ...
                for(unsigned run=0;run<run_count;run++)
                    for(int px=runs[run].x0;px<runs[run].x1;px++)
                        group_pixel(scratch.tile,coverage,px-x0,command,px,py,has_dither,child_dither,
                                    dither_pixels);
            }else for(int x=0;x<count;x++)if(covers(command,x0+x,py))
                group_pixel(scratch.tile,coverage,x,command,x0+x,py,has_dither,child_dither,dither_pixels);
            KSN_PROF_END(blend);}
        }
        {KSN_PROF_BEGIN();
        for(int x=0;x<count;x++){
            ...
            pixels[index]=group_over(pixels[index],scratch.tile[x],opacity,dither,x0+x,py);
        }
        KSN_PROF_END(blend);}
    }
    return KSN_OK;
