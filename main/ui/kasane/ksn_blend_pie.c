 ...
 *  - Cost, in instructions a block and per call, is in docs/perf/kasane-blend-pie.md
 *    with the commands that produced it.
 */
#include <stdint.h>
#include "ksn_types.h"
...
/* The assembly lands with its instruction-level test in the next commit; until
 * then every build runs the lane model, which the model file has checked. */
...
static void ksn_blend8_thin(uint16_t *pixels,int blocks,const int16_t *args){
    ...
    __asm__ volatile(
        ...
        "ee.vmul.u16     q5, q5, q0\n"
        "ee.vldbc.16     q0, %[kp], 2\n"
        "ee.vmul.u16     q1, q5, q0\n"
        /* green: q2 = g8, partial in q1 */
        "ee.vldbc.16     q4, %[pg]\n"
