...
void ksn_frost_init(ksn_frost *frost){if(frost)memset(frost,0,sizeof(*frost));}
...
ksn_result ksn_frost_span(const ksn_frost *frost,uint16_t y,uint16_t x,uint16_t count,
    ...
    if(prefix)ksn_frost_span_scalar(frost,y,x,prefix,tint,pixels);
    ...
    for(;i+8<=count&&x+i<=228;i+=8){
        ...
        ksn_frost_cell_pie(tile,k);
        memcpy(pixels+i,tile,sizeof(tile));
    }
    if(i<count)ksn_frost_span_scalar(frost,y,(uint16_t)(x+i),(uint16_t)(count-i),tint,pixels+i);
    return KSN_OK;
#else
    return ksn_frost_span_scalar(frost,y,x,count,tint,pixels);
#endif
}
