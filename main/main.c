...
#if CONFIG_POCKET_VM_L1_CLOCKBENCH && (CONFIG_POCKET_UI_TASK_PIN_CORE >= 0)
    // Bench-only: pin ui_task to measure pinned vs. today's unaffinitized
    // xTaskCreate (see docs/vm-l1-clock.md). A normal build never takes this
    // branch -- POCKET_UI_TASK_PIN_CORE defaults to -1 and depends on
    // POCKET_VM_L1_CLOCKBENCH, which is off.
    configASSERT(xTaskCreatePinnedToCore(ui_task,"ui",32768,NULL,5,NULL,CONFIG_POCKET_UI_TASK_PIN_CORE)==pdPASS);
#else
