...
void flower_draw(uint16_t *pixels,int y,int height) {
    ...
    if(y+height>=H&&++prof_frames>=60) {
        ...
        ESP_LOGI("garden","SPLIT3 gate=%d tweaks=%d sq=%d canopy=%d rayfast=%d frames=%u decor=%.2f veg=%.2f (%u rows, %u passes, %u cy/row) "
                 ...
                 g_garden_decor_gate,g_garden_scalar_tweaks,g_flower_fixed_sqrt,g_garden_canopy_pie,g_garden_ray_fast,prof_frames,gar-pix,veg,vegrows/prof_frames,
        ...
        g_garden_ray_fast=!g_garden_ray_fast;
