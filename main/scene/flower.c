...
enum { PEARL, HEART, LEAF, IVORY, GOLD, SEED, INNER };
...
static float *depth;
#define FLOWER_BYTES (sizeof(Petal)*MAX_PARTS+sizeof(float)*FW+32*32)
...
//
// One row draws all three botanicals in turn. CRYSTAL is not in the rotation:
...
// enough that somebody looking at the home screen sees all three inside a
// couple of minutes.
...
// that all three species come up and that every swap is hidden. A per-boot seed
...
// One of the *other* two, so a draw never repeats the plant that is already up:
...
static flower_species_t bloom_next(flower_species_t from) {
    unsigned here=(unsigned)from-FLOWER_VALLEY;          // 0..2 among botanicals
    unsigned step=1+bloom_random()%2;                    // 1 or 2, never 0
    return (flower_species_t)(FLOWER_VALLEY+(here+step)%3);
}
...
static uint16_t shade(V n,int petal,V hit) {
    ...
    if(material>=LEAF) {
        ...
        } else if(material==GOLD) {r=255;g=165+25*longitudinal;b=13;}
...
// The sky, in one place, because ray_row blends towards it and flower_draw
// paints it: two copies of this gradient would drift.
static uint16_t sky_at(int py) { return rgb(3,6+py/35,11+py/27); }
        ...
        for(int x=p->xmin;x<=p->xmax;x++) {
            ...
            if(p->shape) {
                ...
                if(bell_hit(p,dx,dy,&z,&n)) {
                    depth[x-X0]=z;
                    row[x]=dissolve(sky,shade(n,i,(V){dx+p->c.x,dy+p->c.y,z}));
                }
                continue;
            }
...
void flower_draw(uint16_t *pixels,int y,int height) {
    ...
    for(int j=0;j<height;j++) {
        ...
        for(int x=0;x<W;x++)row[x]=sky_at(py);
