...
solar_time_sample_t solar_time_now(double demo_elapsed_seconds) {
    ...
    sys_clock_state now;
    if(!sys_device_clock_read(&now)) {
        if(now.health==SYS_CLOCK_UNAVAILABLE)sample.source=SOLAR_TIME_UNAVAILABLE;
        if(now.health==SYS_CLOCK_OUT_OF_RANGE)sample.source=SOLAR_TIME_OUT_OF_RANGE;
        return sample;
    }
    ...
    if(seconds<INT64_C(946684800)||seconds>=INT64_C(2524608000)) {
        sample.source=SOLAR_TIME_OUT_OF_RANGE;
        return sample;
    }
