...
// TEMPORARY A/B switch for the asynchronous transfer below, flipped by the PERF
// report so both paths run in ONE binary (builds differ by up to 15% from code
...
esp_err_t board_present(int y, int rows, uint16_t *pixels) {
    ...
    // which is what the device's queue_size=1 allows and all this needs.
    // Then the strip just drawn is copied into the panel's own buffer and
    // queued. The copy is the price of not touching the 17 call sites that hold
    // `shared` for a whole frame, and it is ~10 us against the ~440 us skipped.
    esp_err_t e = tx_reap();
