...
`sdkconfig.vmclockbench.defaults`（無 pin）と
`sdkconfig.vmclockbench.pin1.defaults`（ui タスクを core 1 に pin）の
2ビルドだけがこれを有効にする。通常ビルド（`sdkconfig.defaults` のみ）は
...
**残作業:** ユーザーがボードの USB を一度抜き差し（または電源再投入）した後、
`idf.py -B build_bench_pin1 -D SDKCONFIG=build_bench_pin1/sdkconfig -D "SDKCONFIG_DEFAULTS=sdkconfig.defaults;sdkconfig.vmclockbench.pin1.defaults" -p <port> flash`
で焼き、`tools/capture_home.py` と背景の `PERF` ログ、`tools/memlog.py --port --check`
で `build_bench_clock`（無pin）と比較すれば、この節を完成できる。
...
# pin 版（実行時の比較用。§2の「残作業」）
idf.py -B build_bench_pin1 -D SDKCONFIG=build_bench_pin1/sdkconfig `
  -D "SDKCONFIG_DEFAULTS=sdkconfig.defaults;sdkconfig.vmclockbench.pin1.defaults" -p <port> flash
