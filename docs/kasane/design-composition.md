...
2026-09-14追記: 更新・cache・modalの利用窓口と取消順序は[利用API v0.3](design-api.md)に統合した。
native coordinatorは実装済み。QuickJS/実アプリのowner境界への接続は引き続き残る。
...
静止比較後に600フレームの連続描画を行う。実時間に応じて背景の市松模様をスクロールし、2枚の半透明矩形の位置とalphaを変え、すりガラスパネルの位置・tint alphaも変える。
全フレームで背景生成→1/8縮小→blur→bilinear拡大→tint→装飾→全17帯転送を実行する。半径1/2は60フレームごとに切り替え、snapshot再利用による負荷軽減は行わない。
目標周期は33,333 µs。期限に間に合わなければmissと失われた周期数を記録し、追いつくための連続catch-upはしない。描画と準備の平均、フレーム時間の平均/p95/最大、実経過時間を記録する。
0/299/599フレームの転送前画素を別途再描画して回収し、その回収時間は計測とアニメーション時刻から除外する。異なる3時点を独立Python参照式と比較する。
free heapと最大連続空きは60フレームごとに採取する。最小値は採取点の最低値であり、一時的な最低値の保証ではない。
計測用に2,400 Bのスタック配列へ600件のフレーム時間を保存し、測定終了後に割当なしでsortする。これは効果領域2 KiBと別の診断計測コストである。
PASSは600フレームを処理して復帰した意味で、30 fps達成を意味しない。達成可否はmiss/p95/実fpsで判断する。
この処理はnativeの画素生成・透過合成・frostフィルタ・SPIの負荷試験で、QuickJS、DS命令bankのPATCH、Wi-Fi/音声併用の総合負荷試験ではない。
