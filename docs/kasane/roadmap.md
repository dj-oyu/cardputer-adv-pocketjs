...
| **研究（v1の阻止条件ではない）** | 33 ms可視native source、copy削減の選択的最適化、binary text PIE、cache/instance、非overlay modal、group opacity・dither等の出荷アプリ未使用効果、制作時schema/生成器。実装/APIやhost試験があっても製品能力を意味しない。 | それぞれ別の画素・CPU・RAM・音声ゲートを先に定義してから採否を決める。33 msを15 Hzに読み替えず、copy回数だけで所有権や修復契約を変更しない。overlay modal は現行契約でUNSUPPORTED。 |
...
| **K4 研究の分離 — 完了** / P4・PIE | 下のR1〜R3として、選択的copy削減・可視33 ms source・text PIEをv1の判定から独立させた。PIEは現music既定OFF。 | R1は一律のcopy回数目標ではなく、対象経路ごとに利益を判定する。R1〜R3の開始条件を満たす具体的な製品要求または利益見込みが出るまで、製品v1を待たせず、危険な所有権変更や無条件の30 Hz更新に着手しない。 |
...
| --- | --- | --- |
| R1 選択的copy削減 — 努力目標 | まず実装とESP32-S3生成命令から不要な走査・呼出しを特定し、契約とメモリを保つ小変更をhostで検証する。その後、対象更新の実機頻度・CPU/RAM寄与を測って採否を決める。所有権変更など高リスク案では、さらにpayload lineage、各destinationのpeak保持量と利益上限を先に示す。既測のview decode単体約5.8〜5.9 µs/frameを全経路の上限と誤用しない。 | 利益が小さければ不採用で終了。候補を実装するならborrow pointer/2 bank/UAFの契約、RAM/断片化、repair、画素、音声、p99を同時に比較。 |
...
R1の実装監査（2026-09-26）: native sourceのpayloadはpin中に借用し、受理時に各core destinationへcopyする。`ksn_source_acquire`のslot metadata配列copyとmusicの`ksn_source_borrow_identity`による直接借用は区別する。次の候補は**改善余地**であり、速度改善の実測結果ではない。

| 候補 | 現状と次の判定 |
| --- | --- |
| JS `view.set` | 候補値配列のcopyはpreflight用に残すが、差分比較・commit・一時文字列解放を全24 slot走査から入力slotだけへ変更した。1-slot更新の比較は最大24→1回。旧/新の同一`-Os` ESP32-S3 objectで`js_schema_set`静的命令数は506→501（実行命令数や速度ではない）。atom照合ならproperty名の文字列化と最大24回の`strcmp`も省けるが、slotごとのatom保持とmount/reset時の寿命・RAMが増えるため別判断。次に実機でCPU/heapと更新頻度を測る。musicのJS更新は毎frameではない。 |
| musicのmodel/plan | `music_refresh`はowned値構造体を一時copyし、presenterもtext配列をplanへcopyする。45秒の既計数は`music_model_copy` 103回/11,948 B、`music_plan` 1,227回/10,636 B。限定light PATCHは既にplan構築を省く。borrow化で増える寿命条件に見合う利益がある場合だけ変更する。 |
| core text-changing PATCH | 同layerの未変更textを候補bankへ複製する場合が残る。現music 45秒では`core_clone_text=0`。単純slot COWは2 bankの断片化と返却済みpointerを壊すため不採用のまま。該当workloadが現れ、CPU/RAM利益が十分な場合だけ所有権設計から再検討する。 |
| 毎frameのview展開 | `render_decode_view`は約498 B/frameだが単独費用は約5.8〜5.9 µs/frame。現在はフレーム内cacheで再利用する。借用pointer化や読取り構造の再配置は、この利益上限の桁を超える別の実測根拠がなければ優先しない。 |
