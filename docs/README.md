...
| [ray-stall-census.md](perf/ray-stall-census.md) | 記録 | 「rays に余地は無い」をハードウェアカウンタで検算した（`GARDEN_RAY_PERF`、XTPERF 11イベント巡回）。CPI 1.75 の内訳は**命令退役0.55 / 命令RAM・ROMビジー0.28 / バブル0.10 / データ待ち0.02**で、**反復除算0.4%・反復乗算0%・命令キャッシュミス0%** —— 算術に余地は無く、3回の試みが算術を触って失敗したのは当然だった。形の違う PIE の pixels カーネルも同じビジーを20%払うので**これは機械の床**。§0 に、`PMCTRL_KRNLCNT` の読み違えで最初の実行が「IPC 0.55」というもっともらしい嘘を返し、`pm0_cy` と ccount を並べた交差検査だけが止めた顛末。§4 に未解決の本丸（描画ループ全体の20%が命令フェッチ待ち） |
