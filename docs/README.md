...
| [flower-shade.md](perf/flower-shade.md) | 記録 | FLOWER の単項最大 `shade`（748 cy/hit）を初めて割った結果と、そこで見つけた死んだ `garden_dither` 呼び出し。`rgbd` が `normal()` より重かった。`garden_dither` を `garden.h` の `static inline` へ移して kernel 32.0 → 29.3 ms（−8.4%）、fps 26.4–27.4 → 28.2–28.4。5種×3時刻のフレームが全バイト一致 |
