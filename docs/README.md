...
| [kasane-text-damage.md](perf/kasane-text-damage.md) | 記録 | テキストの過剰再描画の切り分けと1つ目の修正: 編集欄の1打鍵が全画面17帯64,800Bだったのを、欄の占める帯だけを名指しする `ksn_core_invalidate_bands` へ。残る2つ（damage にX範囲が無い／TEXT の damage が文字列全体）の条件と順序も §5 に |
