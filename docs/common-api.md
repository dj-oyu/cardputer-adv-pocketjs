...
**この節は仕様が先行している。** 現在のファームは旧規則のままで、以下が未実装:

* `shell.c` は `overlay_paint()` を `paint_labels()` の前に呼ぶ。XMBは終了せず、上に描かれる。
* 戻るキーの予約が無い。`overlay_yield()` は定義されているが呼び出し元が無い。
* オーバーレイに入力は届かない。
* `overlay.c` の `REGISTERED[]` は `_Static_assert` で1個に固定され、Settings行はOFF/ONのトグル。オーバーレイを選べるようにする時点で両方外れる。
* モーダルのディスパッチは `tick_run()`（前景ゲスト）にしかない。ホーム画面のループにも要る。
