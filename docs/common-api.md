...
### 3.1 オーバーレイアプリ（提案。未実装）

ホーム画面の背景の**上に**居続けるアプリ。背景の代わりに走る通常アプリと違い、シェルは画面を手放さない。時計・観賞用の表示・シーン定義の編集など「背景を見ながら使うもの」がこの形になる。

```json
{ "runtime": "pocket-overlay", "region": "menu", "required": ["ui.basic"] }
```

**この種別を分ける理由は、合成ではなく統治にある。** 描画の重ね合わせはホーム画面が既に行っている（背景＋メニュー）。新しいのは、**ユーザーがシェルを操作している最中にゲストが生きている**という点で、危険はすべてそこから出る。

#### 制御権限

**入力の所有者はシェルであり、既定でオーバーレイに入力は届かない。** シェルが使うキーは委譲の対象外で、`input.overlay` を宣言しユーザーが許可した場合にのみ、**残りのキーだけ**が配送される。オーバーレイが受け取れないものを列挙するのではなく、シェルが取った後に残ったものを渡す。前者は画面や設定行が増えるたびに穴が開く。

**そして「残り」は、現在のシェルでは空集合である。** 残余の定義は「`shell_key()` が消費しなかったと報告したもの」であり、現状 `shell_key()` はすべての状態ですべてのナビゲーションキーに作用する。唯一の例外は選択リストを開いている間の左右だが、あれは**使い残しではなく「使わないと決めた」キー**で（`shell.c` に明示的な no-op として書かれている）、しかも委譲すれば「ユーザーが設定を変更している最中にだけ届く2つのキー」になる — ゲストに何かを渡す瞬間として最悪である。

したがって `input.overlay` の配線は**存在しない**。規則が間違っているのではなく、規則が指す集合が空である。ここで配線を作れば、それは名前を変えた拒否リストになる。**この節が現実になるのは、シェルが「自分では取らないキー」を持った日**——オーバーレイ専用キーか、`keymap.c` が現在素通ししている数字列——であって、そのときに初めて残余は非空になる。

**オーバーレイは画面遷移を起こせない。** `pocket.app.exit()` は自分の停止だけを意味し、シェルの画面を変えない。モーダルUI・全画面描画・権限プロンプト風の表示を作れない。**描画領域は登録時の `region` に閉じ、はみ出しは切り取るのではなく拒否する**（切り取ると、作者は自分が越えたことに気づかない）。

**シェルのUIを覆えないことは、利便性ではなく安全性の要件。** 権限確認・Wi-Fiパスワード入力・保存の確認を隠せる面は、そこに何を出してもよい面になる。`region` にシェルが自分で使う行を含めない。

**能力の既定はフォアグラウンドのアプリより狭い。** ユーザーがアプリを見ていない間に走るので、音声再生・ネットワーク・外部機器の駆動は宣言と許可があっても既定では止め、明示的に有効化する。**許可の取り消しはホーム画面から常に到達できる**必要があり、オーバーレイが壊れている最中でも到達できなければならない。

#### 安全弁

**前提はホーム画面が生き残ること。** オーバーレイの失敗がシェルを巻き込むなら、ユーザーはそれを止める画面へ行けない。ホームはオーバーレイ抜きで完全に描けなければならず、その経路を検査で保つ。

**確保は起動時に済ませ、途中で足りなくなる形を作らない。** Rust UIコアはメモリ不足を報告せずパニックして再起動する（`ui.setText` が引き起こすフォントアトラス再構築が最大の単発確保）。したがってオーバーレイのUI確保は起動時に予約し、**予約に失敗したら起動を断る**。動き始めてから落ちるより、始まらないほうが良い。JSゲスト自体も、背景シーンが動いている状態で 144KiB を要求することになるので、起動前に空きを確認して足りなければ断る。
