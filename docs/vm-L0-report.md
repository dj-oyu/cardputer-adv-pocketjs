...
| 競合条件 5 種（USB の `P`〜`W`。UI・音声・Wi-Fi を実物の面で動かす） | コミット済み | [apps/vmprobe/condition.js](../apps/vmprobe/condition.js)、[README](../apps/vmprobe/README.md)、`main.c` / `app_session.c` / `vmprobe.h` |
| 実機の採取スクリプト（行列・再起動・厳密な統計） | コミット済み | [tools/vm_l0_capture.py](../tools/vm_l0_capture.py) |
