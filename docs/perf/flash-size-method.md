...
## 3.2 「配置バイト」と「会員が持つセクション総量」は別物 — 0 B の正体

この方法で出る数字（**配置**済みセクションの合計）は「フラッシュを食っている量」であり、
アーカイブ会員の総量ではない。両者を混同すると判定を誤る。実測（`bd0fa43`）:

```
compiler_builtins: 配置 46,329 B  /  破棄（Discarded input sections）162,308 B
                   内訳 .text 141,173 + .literal 15,816 + .rodata 5,319
```

`-Wl,--gc-sections` があるので、会員の中で使われる関数だけが配置され、残りは破棄される。
つまり「会員の大きさ」を数えても意味がなく、**配置バイトを前後で測る**のが正しい。

**0 B になる2つの典型**（どちらもこのキャンペーンで実測した）:

1. **ROM の絶対シンボル（`nm` で `A`、`0x4000xxxx`）** — `__fixdfsi` は 0x400022d4、
   `__divdf3` は 0x40002250。`esp32s3.rom.libgcc.ld` が提供する ROM の実装で、我々のオブジェクトの
   `.text.__divdf3` などは **Discarded input sections 側にしか出ない**。呼び出しを消しても
   フラッシュは 1 B も減らない（浮動小数の除算・変換はこれに当たる）。
2. **会員の他の住人** — 理由行が我々のファイルを指していても、同じ会員の別関数が QuickJS・Rust・
   IDF から使われていれば会員は落ちない（`cgu.13` の 10,244 B は Rust libm が実体で、
   `__fixdfsi` を我々が全部やめても理由行が次の参照元へ移るだけ／`cgu.04` は QuickJS の生きた
   13 関数が握っている）。

判定は必ず「配置バイトを基準マップと前後比較」で行う。`nm` で `A`（絶対）かどうかを最初に見るのが
最短のふるい分けで、`A` なら我々の側の作業は全部無駄になる。
