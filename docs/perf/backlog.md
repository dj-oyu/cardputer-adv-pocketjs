...
下の表は未着手・未決のものだけを残す（`perf/pie-opt` で T1 `bell` 帯棄却・T2 装飾光線の厳密カーネル・T3 MP3 FIR を
取っているので、その3行の状態は plan 側を見ること）。
...
| MP3 デコードのダウンサンプル前FIRのPIE化 | issue #5、`main/pocket/mp3_decode.c:60-72` | 未着手 | PIE版は1サンプル24命令、スカラーは284命令（[obj]）。フレームあたり約1.36ms、デコード本体5.9〜6.7msの2割程度が対象。リングバッファを2倍長にして2箇所へ書けば連続窓になる。`sum/16384`を`sum>>14`にするとPIE版とビット一致し、Cも1サンプル4命令軽くなる（負で端数があるとき出力が1LSBだけ変わる） |
