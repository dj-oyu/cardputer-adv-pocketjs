...
無い。数字はすべて**引用**で、出所（誰が・いつ・どの枝で）を付ける。
→ この段落は 2026-09-16 まで正しかった。**同日、実機を掴んだセッションがこの枝を焼いて測り、
§3b に足した**（板の転送キューと装飾4スイッチの同一バイナリ A/B）。引用の節（§3）はそのまま残す。
...
バイナリが同じ値を出すかは実機で確かめること（§6）。→ **2026-09-16 に取った（§3b）。**

---

## §3b この枝自身の実機計測（2026-09-16、`build_arms`）

実機 Cardputer ADV、`/dev/ttyACM0`、枝 `perf/mp3-fir` `969a8a3` ＋ 装飾 A/B の腕の直し
（§3b-0）。ビルドは `build_arms` = **2,144,112 B**、DIRAM **123,356** / flash **1,557,312**、
実機の空きヒープ `idle_free=269,152`（`memlog.py --port --check` = MEMLOG_OK）。
生ログと解析は `/workspace/knscratch/mp3fir/`（`REPORT.md` が索引）。

**§3b-0 先に直したもの。** FLOWER の 60 フレーム窓はこの枝では `gate`/`tweaks`/`canopy`/`sq` の
**4本を同時に**反転していた。`sq` の出荷値だけ他と逆なので、腕が「3本 ON＋sqrtf」対
「3本 OFF＋固定小数点 sqrt」という混ざった組になり、**ペアの差がどのスイッチにも帰属できなかった**
（incoming は「1つだけ動かす」形だった）。1窓に1本、arm 0 = 出荷の4値という回し方へ戻し、
`SPLIT3` 行の先頭に `arm=` を足した。`tools/flower_instrument_summary.py` の A/B 節も
「各窓を隣の arm 0 窓と対にする」形へ。出荷の設定値は変えていない（arm 0 がそれ）。

**board の LCD 転送キュー**（`async=` が2秒窓ごとに自動で反転。同一バイナリの対）:

| 場面 | draw ms | send ms | fps |
| --- | --- | --- | --- |
| FLOWER | 38.73 → **33.30**（−5.44） | 7.69 → 1.55 | 24.95 → **28.80** |
| FLOWER（969a8a3 素） | 43.88 → **38.16**（−5.72） | 7.87 → 1.86 | 22.15 → **25.30** |
| OCEAN+STARS | 10.24 → **8.18**（−2.06） | 7.12 → 5.05 | 29.7 → 29.7（上限） |

**§3 の引用値「−6.3 ms / 44.7 ms のフレーム」はこの枝で再現した**（−5.4〜−5.7 ms）。
効くのは重い場面で、FLOWER では fps が +15%（キー周期 40 ms の頭打ちに当たらない）。

**装飾の4スイッチ**（FLOWER 330 秒、148 窓、同一種・dissolve 0 の厳密ペア95。差は「腕 − arm 0」、
ms/フレーム、負が速い。`rays`/`rest` は触っていない対照帯）:

| 腕 | スイッチ | n | decor | veg | rays | rest |
| --- | --- | --- | --- | --- | --- | --- |
| 1 | support gate off | 24 | +0.03 | −0.01 | +0.05 | −0.01 |
| 2 | tweaks off | 23 | +0.12 | +0.06 | +0.09 | ±0.00 |
| 3 | **canopy off（スカラー文）** | 24 | **+0.85** | **+0.87** `[+0.70,+1.21]` | −0.01 | ±0.00 |
| 4 | fixed sqrt on | 24 | −0.01 | +0.01 | −0.02 | ±0.00 |
| — | ノイズ床（arm0 対 arm0、5窓差、同一種） | 17 | −0.31 `[−1.44,+1.65]` | −0.01 `[−0.19,+0.08]` | | |

- **動くのは canopy の PIE カーネルだけ**。24 ペア全部が正（最小 +0.70）、8 種のうち 7 種で正、
  対照 `rays`/`rest` は ±0.01。算術も一致: `veg cy/row` **8,071 → 9,803**（+1,732 cy/行 ×
  135 行 = 233,820 cy/frame = **0.97 ms @240 MHz**）。カーネルは 96 命令/8画素 = 12 命令/画素。
- **incoming の「canopy = exact, free, no speed」（154 ペア）とは速度の部分が食い違う。**
  別バイナリ同士なので数字の直接比較はできない（この枝は decor 幅8・`garden_decor_pie=1` で、
  incoming の腕と同じコードではない）。「この枝で同じ問いを聞き直したら +0.87 ms だった」が記録。
- gate / tweaks / 固定小数点 sqrt はこの場面では**ノイズ床以下**。とくに incoming の
  「tweaks で rays −1.46 ms」はこの枝では出ない（+0.09）。
- **未計測**: MP3 FIR の `MP3DEC mean_us`（デバイスの pocketfs に MP3 が無い。`.mp3`/`.wav`/`.pok`
  は storage 2.5 MB に 0 件）、三角関数 LUT のフレーム差（スイッチが無く、予測 0.05 ms は
  この計器の床の下。この変更は `size` の話）、物理の画素（ユーザーの目）。
