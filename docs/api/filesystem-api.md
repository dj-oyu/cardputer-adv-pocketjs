...
pocket.fs.nextFile(path:string,
  options?:{extensions?:string[];wrap?:boolean}):Promise<string|null>;
```
...
`nextFile` は同じフォルダーの**次のファイル**を1つ返す。並び順は大小文字を畳んだ
名前順で、`wrap` を立てると末尾から先頭へ回る。フォルダーはまたがない。

**リストは作らない。** `list` が整列しないのは全件を同時に保持しないためで、その拒否は
正しい。だが「次」を訊くだけなら保持は要らない——1周して「現在より大きい名前のうち
最小のもの」を覚えれば、時間O(n)、メモリO(1)で全順序が手に入る。`wrap` は剰余ではなく
**2周目**として実装されている（1周目はそもそも回る対象を持っていないため）。

**そして判定が1箇所に残る。** 拡張子フィルター・`.` 始まりの除外・`sd_name_reserved`
は `pickFile` が人に見せている行と同じ判定で、アプリが `list` で自前に走査すると
その2つ目の実装ができる。許可の中では `list` が同じ名前を既に見せているので、
これが明かす情報は無い——増えるのは**順序**であって、到達範囲ではない。
