...
本書は初期M1の履歴。以下の操作・未実装範囲・サイズ・RAMは当時の構成を表す。現在は複数アプリ、日本語入力、Playgroundを実装し、フォント領域は512KiBへ変更した。[現行仕様](../platform/architecture.md)と[ビルド手順](../platform/build-environment.md)を優先する。
