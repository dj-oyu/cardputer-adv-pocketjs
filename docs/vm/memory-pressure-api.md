# Memory pressure API — proposed contract (not implemented)
...
offers one listener per foreground guest:
...
mask and episode, sample age, historical episode reason bits, and saturating
failure counters by reason. Optional PSRAM is
reported separately; it is not added to internal DRAM. Headroom and largest
...
Sample guest accounting and internal DRAM at safe owner-task boundaries,
including display-only turns, at most every 100 ms unless a rejection forces
an immediate sample; never traverse all JS objects per frame. `pressure()`
...
clearing. These thresholds are an explicit **pre-enable measurement gate**,
not defaults chosen from an arbitrary utilization percentage. A large failed
...
delivery helper unchanged. An entry failure may have one cooldown retry per
pending state/listener generation; a callback that entered and threw is not
retried, because app-side effects may have occurred. Handler-caused allocation
...
## Acceptance gates before enabling delivery
