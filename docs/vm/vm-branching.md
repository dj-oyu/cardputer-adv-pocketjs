...
`docs/vm/quickjs-freertos-vm-spec.md` の L0〜L5 を進めるためのブランチと作業ツリーの規則。
...
- 常設のツリーは1本で、`.cache/` のジャンクションは本体には無い。**段の作業（`vm/l2c`、
  `vm/segsize` など）は `.claude/worktrees/<段>` に短命の作業ツリーを切り**、統合したら消す。
  そのとき本体の `.cache/` から **`pocketjs`、`native`、`bmi270`、`codecs`、`components`、
  `test262`** の6つをジャンクションで繋ぐ（`mklink /J`）。`test262` を繋がないと
  `test262.py` が固定リビジョン（`72faf8e`）でない欠けたコピーを読み、件数が黙って減る
  （2026-09-14: 4,099 ファイル・7,501 件のはずが 3,584 ファイル・6,511 件に見え、
  「退行 0」のまま基準を読み違えた）。**ジャンクションを張ったツリーを消すときは、先にリンクを
