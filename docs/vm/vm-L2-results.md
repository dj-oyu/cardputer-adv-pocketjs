...
| Enter を1回押して約2秒保持 | `HELLO_COUNT` は **1回だけ**。間隔なし（下の但し書き） |
| Back でホームへ戻る | `APP_STOPPED` → `HOME_READY` |

1行目が本題で、§4.15 が USB で確かめた順序（保存 → stop hook → それぞれの Promise 完了）が、
走査から来たキーでも同じだったことを示す。物理確認はこの3つで、キー全数・同時押し・チャタリングは対象外。

**訂正（同日）**: 初版はこの節に「Enter を約2秒押しっぱなしにすると 4〜14 の11回が出た＝repeat は
毎秒約5回出る」と書いた。**誤り。** 押した本人によれば、指示が「押しっぱなし」と読めず連打していた。
11回は11回の押下であって repeat ではない。**repeat が物理キーで出るかどうかは未確認。**
道具の側の落ち度なので、手順の文言を直し、押下の間隔を出すようにした（repeat なら等間隔で並ぶ）。
やり直した結果は **1回だけ**（`--step 3`、約2秒保持）。

**この1回が意味すること／しないこと**: 押している間に `press` が増えないこと＝**チャタリングも
押下の重複も無い**ことは言える。**repeat が配送されているかは言えない** — hello の購読は
`event.phase === 'press'` だけを数え（`apps/hello/main.js`）、`repeat` の phase は無視するため。
それを確かめるには phase をそのまま記録するアプリか診断が要る。ここでは未確認のままにする。

## 9. TCO と FAIR の既定をどうするか（2026-09-23）

どちらも実装と検査は済んでいて、残っているのは**互換性の判断**だけ、というのが着手時点の整理だった。
判断材料として欠けていたのは FAIR の「実機で本当に効くのか」なので、それを測った。

### 9.1 測る前に見つかった2件（どちらも直した）

- **`F` が VM_PROBE のワークロードに届いていなかった。** `usb_stroke()` は `'F'` を FPU 遅延計測
  （`main/hal/fpu_latency.c`）に先に取っていて、`'A'..'F'` のワークロード分岐より前にあった。
  つまり `tools/vm_l0_capture.py` が F（async_generator）を要求すると、**黙ってパイプライン計測が走り**、
  窓が1つも出ないまま「採取できた」ことになっていた。FPU の行を probe ブロックの後ろへ移した
  （`'K'` が Kasane demo で同じ理由で後ろに置かれているのと同型）。
- **FPU 遅延計測は2回目でパニックしていた。** `F` を続けて2回押すと `LoadProhibited`
  （`lsi_indep`、実測で再現）。ケース関数が読む `a9` は `fpu_latency_run()` が設定していたが、
  **`a9` は呼び出し側のウィンドウにあり**、呼ばれた側が見るのは別のレジスタである。1回目は偶然
  動いていた。スクラッチのポインタを引数で渡す形（`BODY_P`）に直し、出荷構成で3回連続完走を確認。

### 9.2 FAIR（公平順序）の実測（VM_PROBEビルド、各12秒、base）

| ワークロード | 指標 | 互換（FAIR=n） | 公平（FAIR=y） |
| --- | --- | ---: | ---: |
| F async_generator | frame 中央値 / p95 | 3.97 / 7.80 ms | 3.72 / 7.21 ms |
| E io_wait | 完了遅延 中央値 / p95 | 0.03 / 0.05 ms | 0.07 / 0.07 ms |
| hello | frame 中央値 | 0.12 ms | 0.16 ms |
| POCKET PET | frame 中央値 | 0.97 ms | 1.05 ms |
| PET COMPANION | frame 中央値 | 0.23 ms | 0.21 ms |

- **L1 §9.5 の「F型で完了遅延が中央値3〜7倍改善」は、今の負荷では再現できない。** F は完了イベントを
  持たない（`lat_n=0`）ので、公平順序が縮める対象がそもそも無い。E は完了を持つが drain が予算に収まるため、
  公平モードのコードは到達しない — そこで観測できるのは**わずかな費用**（完了遅延 0.03→0.07ms）だけ。
- 実アプリ3本の frame 中央値は雑音の幅（hello +0.04ms、PET +0.08ms、COMPANION −0.02ms）。
- つまり**今のアプリ構成では、公平順序の利益は観測できず、費用だけが見える。**
  利益が出るのは「1論理 drain が予算に収まらず、かつネイティブ完了を待っている」形で、
  出荷している5本はどれもその形ではない。

### 9.3 判断

- **FAIR は n のまま。** 仕様 §12 の順序互換という要求を崩す理由が、今の実測には無い。
  出荷アプリにその形（予算超過の drain ＋ 待っている完了）が現れたら測り直す。
- **TCO も n のまま。** こちらは速度の主張が最初から無く（[vm-tco-design.md](vm-tco-design.md) §10）、
  変わるのは `Error.stack` と再帰の予算境界という観測可能な挙動だけ。有効化の利益は
  「深い末尾再帰を書くアプリが現れたとき」に生じる。
