...
アプリごとの `js=` が F 系列の前から実測(device)で 35.6〜38.9 KB 減（plan §12・§13）。残りは計測の穴。
...
| F2-4 | 全展開の誘発元の計装（どのオブジェクトが、どの操作で全部展開されたか。アプリが列挙で節約を失っていないかを見る） | plan §5.2 | 未着手 |
| F0-a | 床のうちファーム側（`pocket.*` の面など）を計算する。F2 はここにも効いたと見ている（実機の減りがホスト計算より 3〜6 KB 大きい）が内訳は未計測 | plan §2.2・§13.4 | 未着手 |
| F0-b | Kasane デモの外れ回数（`lazyprobe` の stub 不足） | plan §7 | 未着手 |
