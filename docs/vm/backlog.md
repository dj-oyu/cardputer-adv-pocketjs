...
| 3 | **実機で動いた（2026-09-23）。** D1・D3〜D6 達成（移動34回、`max_us` 235 µs、契約一致、smoke 20周、予算内）。**D2 未達成**: pet も companion も 8 ms の予算に届かず一度も駐機しないので、呼び出し元を通らない。D8 未決。詳細は results §3.2〜3.5 | design §8.1、results §3.2 | **一部達成**（旧:  `guest_continue_impl()` の再開直前、USB `&` で武装）。残りは D2 と D8 |
| 7 | **出荷アプリが駐機しないので、移動できる瞬間がほとんど無い**（results §3.3）。`VM_TURN_BUDGET_US`=8,000 µs に対し pet の frame は届かない。L4 が「いつ動かすか」を決めるとき最初に当たる制約。予算を下げるか、駐機以外の移動可能点を作るか、移動を諦めるかの判断が要る | results §3.3 | 未決 |
