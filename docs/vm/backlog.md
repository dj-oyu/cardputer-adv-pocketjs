...
アプリごとの `js=` が F 系列の前から実測(device)で 35.6〜38.9 KB 減（plan §12・§13）。計測の穴は §14 で埋めた。残りは F2-5。
...
| F0-a | 床のうちファーム側（`pocket.*` の面など） | plan §14.1 | 済: 約 6.2 KB（`pocket` の土台 3.2 KB、他は各 0.3 KB 以下）。作成直後の 40.2 KB とホスト計算の差 4.8 KB は未調査 |
| F0-b | Kasane デモの外れ回数 | plan §14.2 | 済: 2.4 回/フレーム（最多は imucal の 8.7） |
| F2-5 | 外れを安くする（リストごとの名前の索引で、入っていない名前はなめずに外れ） | plan §14.4 | 未着手（imucal で 0.07〜0.26 ms/フレーム、推定） |
