...
| 13 | 上流 quickjs-ng 7955cfd49e（中断中コルーチンが closure 経由でしか届かないと GC に回収される UAF）。こちらの全 host 変種で再現（上流テスト3本、asan）。#5 で実機でも GC が走るので到達しやすくなった。移植は JSStackFrame 48B・JSAsyncFunctionData 104B を保つ設計と、L2c の Discard/Terminate の解放順序の確認が必要 | vm-L2-results.md §4.22 | 分析済み・未移植 |
