...
| 3 | **実機の呼び出し元は作ったが、一度も走らせていない。** `guest_continue_impl()` の再開直前、USB `&` で武装、停止時に `VM_RELOC` を出す。`tools/vmtest/device_reloc.py` が駆動するが**未実行**（COM3 が別ラインと共有）。達成すべき目標は design §8.1 の D1〜D9（D1: 1回でも移動する、D6: `max_us` < 5 ms 目標・33 ms 絶対上限、D8: 実機の飛地がホストの深さ由来の値を超えるか） | design §8.1、results §3、§3.2 | 未実施（書き込み待ち） |
