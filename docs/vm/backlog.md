...
[09-relocation-entries.md](vm-ledger/09-relocation-entries.md)。開発は `vm/l3a-refs`。状態は 2026-09-24 時点。

**ホストでの結論**: L3a（動かせる）と L4a（寄せる）は実装・検証済みで既定 n。tlsf モデルで比べた
すべての方式（同サイズで動かす・寄せる・閾値つき・専用領域）が、**動かさない共有ヒープの現行設計に負けた**
（design §10〜§12）。仕様 §12.2-3 に従えば L3/L4 は採らない方向。**確定させるのは下の実機 G12。**

### 実機でやること（次の一歩、COM3 が空き次第）

| # | 項目 | 手順 | 判定 |
| --- | --- | --- | --- |
| 1 | **G12: 実機で断片化が確保を失敗させているか** | 出荷アプリと重い負荷を走らせ、OOM のたびに `JS_TakeOOMCanary` の要求サイズと、その時点の空き総量・最大空きブロックを記録する（`memlog --port`、`MEM free= largest=`） | 「空き総量 ≥ 要求 > 最大空きブロック」の失敗が **1件も無ければ L3/L4 を閉じる**。在れば、その要求が何か（Kasane のアリーナ、JSON、文字列…）を見て、design §10.3 の退避を検討する |
| 2 | D8: 駐機中のネイティブ確保が飛地を増やすか | 駐機しつつ確保もする負荷を作り、`VM_RELOC gap_max` をホスト（深さ由来の値）と比べる | #1 が「不要」なら優先度は下がる。測るだけでよい |

### 開いたままの項目（#1 の結果次第で閉じる）

| # | 項目 | 出典 | 状態 |
| --- | --- | --- | --- |
| 3 | 出荷アプリは 8 ms の予算に届かず駐機しないので、実機では移動の機会がほとんど無い（D2 未達成） | results §3.3 | 記録のみ（動かさない方針なら問題にならない） |
| 4 | D55（床の下に外側の JS 活性が在ると移動を拒む）を緩められるか | design §2.1 | 保留 |
| 5 | 退避（空きを分断しているセグメントを動かして空きを合体させる）。アロケータの隣接情報が要る | design §10.3、D58 | #1 次第 |
